...
public class HtmlLogFileAppenderTest {
    ...
    public void init() throws ResourceUtilsException, ReporterException, IOException {
        when(resourceUtils.getFile(any())).thenReturn(new File(""));
        doNothing().when(resourceUtils).writeToFile(isA(WritableResource.class), anyString(), anyBoolean());
        PowerMockito.mockStatic(Jsoup.class);
        PowerMockito.when(Jsoup.parse(isA(File.class), anyString())).thenReturn(jsoupNodes);
    }
