...
- `etl.dir.format`: format of the path within the working directory for the extracted files. There are three options: `job` will save the study files in `timestamp/study_id` path; `id` will save the study files in `study_id/timestamp` path; and `version` will save the study files in `study_id/version` path, where `version` is the value saved in the "version" property of the study object. 
